...
def speakerDiarization(filename, n_speakers, mt_size=2.0, mt_step=0.2, 
	...
	#print("time duration : ",numpy.array(range(len(cls)))*mt_step+mt_step/2.0)
	print("audio duration : ",duration)
	#mtStep1
	#mt_step = 0.5
	s, c = flags2segs(cls, mt_step)
	print("my s and c : ")
	print(s,c)
	s = s.tolist()
	s = [[round(number,2) for number in numbers] for numbers in s]
	c = [int(number) for number in c]
	c = ["Speaker"+str(number) for number in c]
	...
	def processAudio(WAV_FILE):
		...
		try:
			#speech.append(r.recognize_google(audio,language='en-US',show_all=True))
			wit_results = r.recognize_wit(audio, key=WIT_AI_KEY, show_all=True)
			speech.append(wit_results['_text'])
		except sr.UnknownValueError:
			speech.append("Wit Speech Recognition could not understand audio")
		except sr.RequestError as e:
			speech.append("Could not request results from Wit Speech Recognition service; {0}".format(e))
	...
	while n < len(s):
		for numbers in s:
			firstPart = numbers[0] * 1000
			secondPart = numbers[1] * 1000
			tempObject = sound[firstPart : secondPart ]
			tempObject = silent_segment + tempObject + silent_segment
			myAudioFile = exportPath + "slice" + str(n) +".wav"
			tempObject.export(myAudioFile , format="wav")
			n += 1
			processAudio(myAudioFile)
	print("Total segments processed :" ,n)
	print("Length of speech : ",len(speech))

	df = pd.DataFrame()
	df["[start_time , end_time]"] = s
	df["speakers"] = c
	df["speech"] = speech
	fname = filename.split(".")[0]
	fname = fname + "_diarization_" + str(mt_step) + "_n" + str(n_speakers) + "_wit" + ".csv"
	df.to_csv(fname,index=False)
	print("segments written to csv")
	# for filename in glob.glob("./slice*"):
	# 	os.remove(filename)
	# print("Slices deleted... !!! ")
	...
	if plot_res:
		plt.xlabel("time (seconds)")
		#print("speakers : " ,s_range, "time : ",sil_all)
		if n_speakers<=0:
			plt.subplot(212)
